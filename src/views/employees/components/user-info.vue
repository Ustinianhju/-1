      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <UploadImg ref="uploadAcatar" :default-url="employeesAvatar" @on-success="uploadAvatarSuccess" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <upload-img ref="uploadPics" :default-url="employeesPic" @on-success="uploadPic" />
        </el-form-item>
...
export default {
  ...
  methods: {
    async loadUserInfo() {
      const res = await getUserFateilById(this.userId)
      if (res.staffPhoto) {
    ...
    async saveEmployeesInfo() {
      try {
        if (this.$refs.uploadPics.loading) {
    ...
    async saveUserInfo() {
      try {
        if (this.$refs.uploadAcatar.loading) {
          return this.$message.error('头像还在上传中...')
        }
        await saveUserFateilById(this.userInfo)
        this.$message.success('更新成功')
      } catch (error) {
